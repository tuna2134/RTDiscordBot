...
with open("RT/level.json",mode="r") as f:
    levels = json.load(f)
with open("RT/server.json",mode="r") as f:
    servers = json.load(f)
...
async def add(ctx, arg, *, arg2):
    ...
    if ctx.author.guild_permissions.administrator or ctx.author.id == team_id or ss == True:
        ...
        await ctx.send("このコマンドの実行権限があなたにありません。")
...
@bot.group()
async def exocom(ctx):
    if ctx.invoked_subcommand is None:
        await ctx.send("exocomにはサブコマンドがあり、add,dele,listをexocomの後に追加しなければなりません。\nわからない場合は`rt!help exocom`をチェック！")
        ...
        async with ctx.typing():
            ...
            if arg2 is None:
                if ctx.message.attachments is not None:
                    ...
                    ireru = ""
            with open(f"RT/server/exocom/{ctx.guild.id}.txt", 'a') as f:
                f.write(f"\n{arg}\n{ireru}")
            with open(f"RT/server/exocom/{ctx.guild.id}.txt") as f:
                text = f.read()
            with open(f"RT/server/exocom/{ctx.guild.id}.txt", 'w') as f:
                f.write('\n'.join(filter(lambda x: x.strip(), text.split('\n'))))
            f.close()
        await ctx.send("EXコマンドを登録！")
    else:
        await ctx.send("このコマンドの実行権限があなたにありません。")
...
async def dele(ctx, arg):
    ...
    for si in ctx.message.author.roles:
        if si.name == "RT操作権限":
            ss = True
    if ctx.author.guild_permissions.administrator or ctx.author.id == team_id or ss == True:
        pathh = f"RT/server/exocom/{ctx.guild.id}.txt"
        if not (os.path.exists(pathh)):
            kek = "まだEXコマンドが登録されていません。"
        else:
            with open(pathh) as f:
...
async def list(ctx, arg):
    ...
    if (os.path.exists(pathh)):
        with open(pathh) as f:
            lis = [s.strip() for s in f.readlines()]
        f.close()
        pa = int(arg)
        no = len(lis)
        embed = discord.Embed(title="EXコマンド一覧", description=f"{pa}ページ目\nEXコマンドの数:{len(lis)}", color=0x0066ff)
        c = 0
        cc = 0
        if pa > 1:
            ccc = pa*10
        ...
        await ctx.send("まだEXコマンドが登録されていません。")

@bot.command()
async def mesto(ctx, arg):
    ss = False
    for si in ctx.message.author.roles:
        if si.name == "RT操作権限":
            ss = True
    if ctx.author.guild_permissions.administrator or ctx.author.id == team_id or ss == True:
        if str(ctx.guild.id) in servers:
            servers[str(ctx.guild.id)]["展開"] = arg
        else:
            servers[str(ctx.guild.id)] = {}
            servers[str(ctx.guild.id)]["展開"] = arg
            servers[str(ctx.guild.id)]["level"] = "False"
        with open('RT/server.json', 'w') as f:
            json.dump(servers, f, indent=4)
        await ctx.send(f"メッセージリンク展開機能を`{arg}`にしました。")
    else:
        await ctx.send("このコマンドの実行権限があなたにありません。")

@bot.command()
async def levelto(ctx, arg):
    ss = False
    for si in ctx.message.author.roles:
        if si.name == "RT操作権限":
            ss = True
    if ctx.author.guild_permissions.administrator or ctx.author.id == team_id or ss == True:
        if str(ctx.guild.id) in servers:
            servers[str(ctx.guild.id)]["level"] = arg
        else:
            servers[str(ctx.guild.id)] = {}
            servers[str(ctx.guild.id)]["展開"] = "False"
            servers[str(ctx.guild.id)]["level"] = arg
        with open('RT/server.json', 'w') as f:
            json.dump(servers, f, indent=4)
        await ctx.send(f"レベルﾘｱｸｼｮﾝ通知を`{arg}`にしました。")
    else:
        await ctx.send("このコマンドの実行権限があなたにありません。")
    ...
    if not message.channel.name in nochlist:
        ...
        if exp >= round((4 * (cur_lvl ** 3)) / 5):
            ...
            if str(message.guild.id) in servers:
                if servers[str(message.guild.id)]["展開"] == "True":
                    await message.add_reaction("<:LVUP:734358675736625202>")
        with open('RT/level.json', 'w') as f:
            json.dump(levels, f, indent=4)

    if (os.path.exists(f"RT/server/exocom/{message.guild.id}.txt")):
        with open(f"RT/server/exocom/{message.guild.id}.txt") as f:
            li = [s.strip() for s in f.readlines()]
        f.close()
        for sea in li:
            if li.index(sea) % 2 == 0:
                if mess.count(sea) > 0:
                    nan = li.index(sea) + 1
                    nai = li[nan]
                    await message.channel.send(nai)
